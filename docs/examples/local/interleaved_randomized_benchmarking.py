...
def main():
    ...
    print(f"Using LocalBackend with ~2% depolarizing error per gate\n")
    ...
    interleaved_df = interleaved_result.analyze(plot=True, save_data=True, save_image=True)
    
    print("\n" + "=" * 65)
    print("SUMMARY:")
    print(f"Gate: {interleaved_rb.interleaved_gate.upper()}")
    print(f"Interleaved RB completed successfully!")
    
    # Demonstrate easy access to individual experiment data
    print(f"\nData Access:")
    print(f"Interleaved RB final survival: {interleaved_df.iloc[-1]['mean_survival_probability']:.4f}")
    if interleaved_rb.standard_df is not None:
        print(f"Standard RB final survival: {interleaved_rb.standard_df.iloc[-1]['mean_survival_probability']:.4f}")
    print(f"Sequence lengths tested: {interleaved_rb.sequence_lengths}")
    
    print(f"\n✅ Interleaved RB experiment completed successfully!")
    print(f"📊 Combined plot and analysis data saved automatically")
