...
class CHSHAnalysisResult(BaseModel):
    ...
    quantum_theoretical_max: float = Field(default=2.828, description="Quantum theoretical maximum (2√2)")
    significance: float = Field(description="Statistical significance of violation (in σ)")
    correlations: dict[str, float] = Field(description="Individual correlation values")
    correlation_errors: dict[str, float] = Field(description="Standard errors of correlations")
    measurement_counts: dict[str, dict[str, int]] = Field(description="Raw measurement counts")
